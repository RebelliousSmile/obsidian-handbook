...
- Restore the schema-declared Monsterhearts row layout, including the reserved central portrait cell and vault-resolved `playbookImage` assets.
- Render Monsterhearts creation data semantically without leaking schema field names or repeating option lists already present in the editorial copy.
...
- Accept canonical three-key protocol-1 manifests and derive adjacent candidate evidence without a legacy runner field.
...
- Add schema-in-the-mist adoption proof to the shared release-train interface.

## [2.26.0] - 2026-09-22

### Added

- Add the canonical schema-pbta release-train manifest proof and validated consumer provenance evidence.
...
- Align release metadata with the published changelog version.
...
- Add shared visual Clock, Move, and NPC Reaction callouts to every published PbtA pack, with pack-specific styling including Monsterhearts.

### Fixed

- Restore Handbook validation against the published PbtA v8 contract and frozen lockfile; keep the layout projection compatible with the project's TypeScript target.

## [2.20.0] - 2026-09-21

### Added

- Add the generic Handbook projection for schema-published region columns, including deterministic handling of absent and unassigned regions.
...
- Give explicit note-local layout regions the full reading width in Adrenaline and PbtA themes, so the theme's editorial columns cannot split them.
- Keep three local columns until the note content area narrows to 520 px.

## [2.15.2] - 2026-09-16

### Fixed

- Switch note-local columns to a single column when the reading pane is narrow, even if the Obsidian window is wide.

## [2.15.1] - 2026-09-16

### Fixed

- Render note-local Markdown columns in Obsidian reading view and restore them after Obsidian refreshes the rendered blocks.
...
- Render monster capabilities in distinct, ordered groups while preserving the PJ sheet grid and vertical PNJ/monster profiles.
- Assert representative Adrenaline statblocks, their mobile layout guards, and their long-content wrapping rules.
...
- Externalize PostCSS from ESM assertion harnesses.

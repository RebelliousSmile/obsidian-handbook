...
Every supported provider path then builds the production Handbook bundle and loads it in the explicitly provisioned Obsidian 1.13.7 host. Passed evidence lists `production-build` and `obsidian-plugin-load`, the renderer-observed host version, the plugin manifest version, and SHA-256 for `main.js`, `manifest.json`, and `styles.css`. The runner writes the manifest inside a disposable detached checkout; Handbook does not modify committed package, lock, or source-pack inputs. It atomically writes evidence beside the manifest only after provider and host proofs pass; any rejected manifest, missing host, build failure, or activation failure leaves no passed evidence.

...
Every supported provider path then builds the production Handbook bundle and loads it in the explicitly provisioned Obsidian 1.13.7 host. The provider-owned protocol vocabulary requires `commonjs-plugin-build` and `obsidian-1.13.7-plugin-load`. Public evidence keeps the closed six-key shape `protocol`, `status`, `candidate`, `consumer`, `lock`, and `journey`; renderer identity, plugin identity, trust state, exceptions, and SHA-256 for `main.js`, `manifest.json`, and `styles.css` remain in the focused host journey's local report. The runner writes the manifest inside a disposable detached checkout; Handbook does not modify committed package, lock, or source-pack inputs. It atomically writes evidence beside the manifest only after provider and host proofs pass; any rejected manifest, missing host, build failure, or activation failure leaves no passed evidence.
